...
```bash
etl-pipeline$ sam build --use-container
```
...
```bash
etl-pipeline$ sam logs -n StockCheckerFunction --stack-name "etl-pipeline" --tail
```
...
```bash
etl-pipeline$ pip install -r tests/requirements.txt --user
# unit test
etl-pipeline$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
etl-pipeline$ AWS_SAM_STACK_NAME="etl-pipeline" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "etl-pipeline"
```
